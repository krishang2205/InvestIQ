...
const ReportView = ({ data, onBack }) => {
    ...
    const StatusPill = ({ label, value, type }) => {
        ...
        return (
            <div style={{
                ...
                <span style={{ fontSize: '0.75rem', color: 'var(--color-secondary)', textTransform: 'uppercase' }}>
                    {label}
                </span>
                <span style={{ fontSize: '0.875rem', fontWeight: 600, color: textColors[type] || textColors.neutral }}>
                    {value}
                </span>
            </div>
        );
    ...
    const chartData = Array.isArray(data?.priceBehavior?.chartData) ? data.priceBehavior.chartData : [];
    const prices = chartData.map(d => Number(d.price)).filter(v => Number.isFinite(v));

    const hasChart = prices.length >= 2;

    const maxPrice = hasChart ? Math.max(...prices) * 1.1 : 1;
    const minPrice = hasChart ? Math.min(...prices) * 0.9 : 0;

    const points = hasChart
        ? chartData.map((d, i) => {
            const p = Number(d.price);
            const x = (i / (chartData.length - 1)) * PriceWidth;
            const y = PriceHeight - ((p - minPrice) / (maxPrice - minPrice)) * PriceHeight;
            return `${x},${y}`;
        }).join(' ')
        : "";

    // ✅ dynamic “selected stock” for peer highlight (first row is usually the selected symbol/company)
    const primaryPeerName = data?.header?.company || "";
        ...
        <div style={{ maxWidth: '900px', margin: '0 auto', paddingBottom: '6rem' }}>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <div>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem', fontSize: '0.75rem', color: 'var(--color-secondary)', marginTop: '0.25rem' }}>
                            ...
                            <span style={{ padding: '0 0.5rem', background: 'rgba(255,255,255,0.1)', borderRadius: '4px' }}>
                                {data?.header?.exchange || "—"}
                            </span> •
                            <span style={{ padding: '0 0.5rem', background: 'rgba(255,255,255,0.1)', borderRadius: '4px' }}>
                                {data?.header?.sector || "—"}
                            </span>
                ...
                <div style={{ textAlign: 'right' }}>
                    <p style={{ fontSize: '0.875rem', fontWeight: 600, margin: 0 }}>
                        {data?.header?.generatedOn || ""}
                    </p>
                    <p style={{ fontSize: '0.75rem', color: 'var(--color-secondary)', margin: 0 }}>
                        Range: {data?.header?.dataRange || "—"}
                    </p>
                    <span style={{ fontSize: '0.65rem', background: '#333', padding: '0.1rem 0.4rem', borderRadius: '4px', color: '#aaa' }}>
            ...
            <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: '12px', marginBottom: '2rem' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '2rem', alignItems: 'start' }}>
                    <div>
                        <p style={{ color: 'var(--color-text)', lineHeight: '1.6', marginBottom: '1rem' }}>
            ...
            <div style={{ marginBottom: '2.5rem' }}>
                <h3 style={{ fontSize: '1.25rem', marginBottom: '1rem', borderLeft: '4px solid var(--color-accent)', paddingLeft: '0.75rem' }}>
                    Price Behavior
                </h3>
                <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: '12px' }}>
                    ...
                    <div style={{ height: '200px', width: '100%', display: 'flex', alignItems: 'flex-end', position: 'relative' }}>
                        ...
                        {/* Axis labels mocked */}
                        <div style={{ position: 'absolute', bottom: '-20px', left: 0, fontSize: '0.75rem', color: 'var(--color-secondary)' }}>
                            Start
                        </div>
                        <div style={{ position: 'absolute', bottom: '-20px', right: 0, fontSize: '0.75rem', color: 'var(--color-secondary)' }}>
                            Latest
                        </div>
                    </div>

                    <p style={{ marginTop: '1.5rem', fontSize: '0.875rem', color: 'var(--color-secondary)', lineHeight: 1.6 }}>
                    ...
                    {(data?.technicalSignals || []).map(sig => (
                        <div key={sig.name} className="glass-panel" style={{ padding: '1rem', borderRadius: '8px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                                ...
                                <span style={{ fontSize: '0.75rem', color: sig.status === 'Bullish' ? '#34d399' : '#facc15' }}>
                                    {sig.status}
                                </span>
                            </div>
            ...
            {/* 10. INDUSTRY & FINANCIAL DEEP DIVE */}
            <div style={{ marginBottom: '2.5rem' }}>
                <h3 style={{ fontSize: '1.25rem', marginBottom: '1rem', borderLeft: '4px solid var(--color-accent)', paddingLeft: '0.75rem' }}>
                    {data?.industryOverview?.title || "Industry Overview"}
                </h3>
                ...
                <h3 style={{ fontSize: '1.25rem', marginBottom: '1rem', borderLeft: '4px solid var(--color-accent)', paddingLeft: '0.75rem' }}>
                    {data?.financialAnalysis?.title || "Financial Deep Dive"}
                </h3>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                        ...
                        <tbody>
                            {(data?.peerComparison || []).map((peer, i) => {
                                const isPrimary =
                                    i === 0 ||
                                    (primaryPeerName && peer.name && peer.name.toLowerCase().includes(primaryPeerName.toLowerCase()));

                                return (
                                    <tr key={`${peer.name}-${i}`} style={{ borderBottom: i !== data.peerComparison.length - 1 ? '1px solid var(--glass-border)' : 'none' }}>
                                        <td style={{ padding: '1rem', fontWeight: 600 }}>
                                            {peer.name}
                                        </td>
                                        <td style={{ padding: '1rem' }}>{peer.price}</td>
                                        <td style={{ padding: '1rem', color: isPrimary ? 'var(--color-primary)' : 'inherit' }}>
                                            {peer.pe}
                                        </td>
            ...
            <div style={{ marginBottom: '4rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                <div style={{ padding: '1.5rem', background: 'rgba(59, 130, 246, 0.1)', borderRadius: '12px', border: '1px solid rgba(59, 130, 246, 0.2)' }}>
                    <span style={{ fontSize: '0.75rem', fontWeight: 700, letterSpacing: '1px', color: '#60a5fa', textTransform: 'uppercase', display: 'block', marginBottom: '0.5rem' }}>
                        Tactical View (0-3 Months)
                    </span>
                    <p style={{ fontSize: '0.9rem', lineHeight: '1.6' }}>
                        {data?.outlook?.shortTerm || "—"}
                    </p>
                </div>
                <div style={{ padding: '1.5rem', background: 'rgba(16, 185, 129, 0.1)', borderRadius: '12px', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
                    <span style={{ fontSize: '0.75rem', fontWeight: 700, letterSpacing: '1px', color: '#34d399', textTransform: 'uppercase', display: 'block', marginBottom: '0.5rem' }}>
                        Structural View (12+ Months)
                    </span>
                    <p style={{ fontSize: '0.9rem', lineHeight: '1.6' }}>
                        {data?.outlook?.longTerm || "—"}
                    </p>
                </div>
            </div>
